...
def onSegment(px, py, qx, qy, rx, ry):
    if max(px, rx) >= qx >= min(px, rx) and max(py, ry) >= qy >= min(py, ry):
        return True
    return False
...
# edge [x1,y1,x2,y2]
edge = [[(0.3, 0.3), (8, 0.3)], [(8.8, 0.3), (10.8, 0.3)], [(10.8, 0.3), (10.8, 5.3)], [(10.1, 5.3), (10.8, 5.3)],
        [(10.1, 5.3), (10.1, 6.3)], [(0.3, 6.3), (10.1, 6.3)], [(0.3, 0.3), (0.3, 6.3)], [(0, 0), (8, 0)],
        [(8, 0), (8, 0.3)], [(8.8, 0), (8.8, 0.3)], [(8.8, 0), (11.1, 0)], [(0, 6.6), (11.1, 6.6)],
        [(11.1, 0), (11.1, 6.6)], [(2.5, 2), (8, 2)], [(2.5, 2), (2.5, 2.5)], [(2.5, 2.5), (7.5, 2.5)],
        [(7.5, 2.5), (7.5, 3.5)], [(2.5, 3.5), (7.5, 3.5)], [(2.5, 3.5), (2.5, 4)], [(2.5, 4), (7.5, 4)],
        [(7.5, 4), (7.5, 5)], [(2.5, 5), (7.5, 5)], [(2.5, 5), (2.5, 5.5)], [(2.5, 5.5), (8, 5.5)], [(8, 2), (8, 5.5)],
        [(-2, -2), (11.1, -2)], [(11.1, -2), (11.1, -2.3)], [(-2.3, -2.3), (-2.3, 6.6)], [(-2, -2), (-2, 6.6)],
        [(-2.3, 6.6), (-2, 6.6)], [(0, 0), (0, 6.6)], [(-2.3, -2.3), (11.1, -2.3)], [(-2, 6.6), (0, 6.6)],
        [(11.1, -2), (11.1, 0)]]
edge = np.array(edge)
sprawn = [[8.4, 0.5], [6, 1.6], [3, 1.6], [3, 3], [6, 3], [3, 4.5], [6, 4.5], [3, 5.8], [6, 5.8], [10, 4], [10, 2],
          [-1.3, -1.3], [1, 4], [5, -1], [-1, 5]]
sprawn = np.array(sprawn)
# ax=plt.subplot(1,1,1)
user_loc = np.zeros((10, 2), dtype=float)
# choose 10 out of 15 location to activate
chosen_loc = random.sample(range(0, 15), 10)
for x in range(0, 10):
...
def main(duration, T_time, counter, threshold,test_num):  # Changed
            ...
            while not passed:
                if direction[y] == 1:
                    ...
                    passed = intersection_test(user_loc[y, 0], user_loc[y, 1], user_loc[y, 0] + 0.707 * speed / duration,
                                               user_loc[y, 1] + 0.707 * speed / duration)
                    # print(passed)
                    if not passed:
                        direction[y] = random.randint(1, 8)
                        # print(y,"	",direction[y])
    ...
    save = "1/track" + str(test_num) + ".npy" # Changed
...
def test(duration, T_time, counter, threshold, tests_cnt): # Changed
    res = []
    for _ in range(tests_cnt):
        res.append([0.0,0.0,0.0])
    for i in range(tests_cnt):
        res[i] = main(duration, T_time, counter, 1.0,(i+1)) # Changed
    for i in range(tests_cnt):
        res[i] = main(duration, T_time, counter, 1.3,(i+26)) # Changed
    for i in range(tests_cnt):
        res[i] = main(duration, T_time, counter, 1.5,(i+51)) # Changed
    for i in range(tests_cnt):
        res[i] = main(duration, T_time, counter, 2.0,(i+76)) # Changed
    return res
...
def MAIN():
    ...
    for i in range(lTt):
        for j in range(lthre):
            ...
            result = test(10, Tt[i], 10, thre[j], 25) # pos, neg, mix
